...
float3 col;
                col = tex2D(_ColorMap,v.uv*2+shadow*10 ) ;//2 * tCol * abs( refl * .3 + .7) * hsv( _Hue +v.uv.y * .3 ,1,1);//lerp(tCol , tex2D(_ColorMap , float2(pow( m,4) * 4 + _Swap * .3,0)) , .6+pow(m,10));// * (fCol * .3 + .7);
        
            if( length(col) > .9){ col = .9;}else{ col = .3;}
            if( shadow > 0.2){discard;}
