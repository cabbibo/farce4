...
float3 derivedNormal = cross( normalize(posddx), normalize(posddy) );
    ...
    float3 n = v.nor;//derivedNormal;//  v.nor * nTex.y  + v.tan * nTex.x * 10+ v.bi * nTex.y*10;////normalize( normalize(float3( noise(v.worldPos*_NoiseSize) , noise(v.worldPos*_NoiseSize+10000) , noise(v.worldPos*_NoiseSize +100) -.5)) - 6*v.nor);
           // n = v.nor;//normalize(n);
...
float3 col;
                col = v.uv.y;//dot( _WorldSpaceLightPos0.xyz , v.nor ) + .5;// * .5 + .5;//2 * tCol * abs( refl * .3 + .7) * hsv( _Hue +v.uv.y * .3 ,1,1);//lerp(tCol , tex2D(_ColorMap , float2(pow( m,4) * 4 + _Swap * .3,0)) , .6+pow(m,10));// * (fCol * .3 + .7);
