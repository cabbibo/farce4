    ...
    SubShader
    {
      ...
      Tags { "LightMode" = "ForwardBase" "RenderType"="Opaque" "Queue"="Geometry+1" "ForceNoShadowCasting"="True"  }
            ...
            float4 frag(vertexOutput v) : COLOR
            {
                ...
                col = clamp( 1-abs(attenuation2- attenuation) * .6 , 0, 1);//hsv(m * (.5 * m2 + .5),1,1);//+ .4*attenuation;// tex2D(_ColorMap,float2(m * _Size + _Offset,.5)) *  (m * .5 + .5);//lerp( float3(0,1,0) , float3(0,0,1) , 1-m);// * float3(0,1,0);
