    ...
    SubShader
    {
      ...
      Tags { "LightMode" = "ForwardBase" "RenderType"="Opaque" "Queue"="Geometry+1" "ForceNoShadowCasting"="True"  }
            ...
            float4 frag(vertexOutput v) : COLOR
            {
                ...
                float size = 1000;
                float n = noise(v.world * 120 + _Time.y );//sin(v.world.x * size)  + sin(v.world.y*size) + sin(v.world.z * size) ;
                ...
                col = m;//clamp( 1-abs(attenuation2- attenuation) * .6 , 0, 1);//hsv(m * (.5 * m2 + .5),1,1);//+ .4*attenuation;// tex2D(_ColorMap,float2(m * _Size + _Offset,.5)) *  (m * .5 + .5);//lerp( float3(0,1,0) , float3(0,0,1) , 1-m);// * float3(0,1,0);
