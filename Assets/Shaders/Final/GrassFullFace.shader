  ...
  Properties {
    ...
    SubShader {
      ...
      float _BaseHue;
       ...
       float hue = v.debug.x;//.3 + tcol.r   * .1;//  +saturate(length(v.vel) * 20) * .1 ;
       color.xyz = tex2D(_ColorMap , float2( sin(v.debug.x * 1000),0 ));// * (v.uv.y * .5 + .8);;
       
        //color.xyz *=  v.uv.y * 1.1;
        //color.xyz = thsv(v.debug.x * .2,1,1);
